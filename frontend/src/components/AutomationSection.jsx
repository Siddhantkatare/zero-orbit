    ...
    <section className="py-20 bg-gray-900 text-white relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="text-center mb-16">
          <div className="inline-flex items-center px-4 py-2 bg-gray-700/30 text-gray-300 rounded-full text-sm font-medium mb-4">
            <Zap className="w-4 h-4 mr-2" />
            {automation.badge}
          </div>
          <h2 className="text-4xl lg:text-5xl font-bold mb-6">
            {automation.title.split(' ').map((word, index) => (
              <span key={index} className={index === automation.title.split(' ').length - 1 ? 'text-white' : ''}>
                {word}{' '}
              </span>
            ))}
          </h2>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8 mb-16">
          {automation.features.map((feature, index) => {
            ...
            return (
              <div
                key={index}
                className="group bg-gray-800/50 backdrop-blur-sm rounded-2xl p-6 border border-gray-700/50 hover:border-white/50 transition-all duration-300 transform hover:-translate-y-2"
              >
